...
namespace DataModule.Redis
{
    public static class RedisTool{
        private static readonly string Coonstr = "127.0.0.1:6379";
        private static object _locker = new Object();
        private static ConnectionMultiplexer _instance = null;
        private static bool isPBOrJson;
        public static void SetPB(bool isPB) {
            isPBOrJson = isPB;
        }
        /// <summary>
        /// 默认的 Key 值（用来当作 RedisKey 的前缀）
        /// </summary>
        private static readonly string preKey = "";
        /// <summary>
        ...
        public static T Get<T>(string key) {
            key = MergeKey(key);
            if (isPBOrJson)
                return DataMethods.PBDes<T>(GetDatabase().StringGet(key));
            else return DataMethods.JsonDes<T>(GetDatabase().StringGet(key));
        }
        /// <summary>
        /// 根据key获取缓存string
        /// </summary>
        ...
        public static void Set(string key, object value) {
            key = MergeKey(key);
            if(isPBOrJson)
            GetDatabase().StringSet(key, DataMethods.PBSer(value));
            else GetDatabase().StringSet(key, DataMethods.JsonSer(value));
        }
        /// <summary>
        /// 设置缓存string
        /// </summary>
        ...
        public static async Task SetAsync(string key, object value) {
            key = MergeKey(key);
            if (isPBOrJson)
                await GetDatabase().StringSetAsync(key, DataMethods.PBSer(value));
            else await GetDatabase().StringSetAsync(key, DataMethods.JsonSer(value));
        }
        ...
        public static async Task<string> GetAsync(string key) {
            key = MergeKey(key);
            string value = await GetDatabase().StringGetAsync(key);
            return value;
        }
        ...
        /// <param name="key"></param>
        /// <returns></returns>
        public static long Increment(string key, long v = 1, CommandFlags flag = CommandFlags.None) {
            key = MergeKey(key);
            //三种命令模式
            ...
            return GetDatabase().StringIncrement(key, value: v, flags: flag);
        }
        /// <summary>
        /// string方式实现递减
        /// </summary>
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static long Decrement(string key, long v = 1, CommandFlags flag = CommandFlags.None) {
            key = MergeKey(key);
            return GetDatabase().StringDecrement(key, value: v, flags: flag);
        }
        #endregion
        /// <summary>
        /// 判断在缓存中是否存在该key的缓存数据
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static long HashIncrement(string key, string value,long v=1, CommandFlags flag = CommandFlags.None) {
            key = MergeKey(key);
            return GetDatabase().HashIncrement(key, value,value:v, flags: flag);
        }
        /// <summary>
        /// hash方式实现递减
        /// </summary>
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static long HashDecrement(string key, string value, long v=1,CommandFlags flag = CommandFlags.None) {
            key = MergeKey(key);
            return GetDatabase().HashDecrement(key, value, value:v,flags: flag);
        }
        #endregion
